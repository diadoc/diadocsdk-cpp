...
const AcceptanceCertificateSellerTitleInfo& AcceptanceCertificateSellerTitleInfo::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_Invoicing_2fAcceptanceCertificateInfo_2eproto();  return *default_instance_;
}
...
bool AcceptanceCertificateSellerTitleInfo::IsInitialized() const {
  ...
  for (int i = 0; i < works_size(); i++) {
    if (!this->works(i).IsInitialized()) return false;
  }
  if (has_signature()) {
