...
const ForwardDocumentRequest& ForwardDocumentRequest::default_instance() {
  if (default_instance_ == NULL) protobuf_AddDesc_Forwarding_2fForwardingApi_2eproto();  return *default_instance_;
}
...
bool GetForwardedDocumentsRequest::IsInitialized() const {
  ...
  for (int i = 0; i < forwardeddocumentids_size(); i++) {
    if (!this->forwardeddocumentids(i).IsInitialized()) return false;
  }
  return true;
}
...
bool GetForwardedDocumentsResponse::IsInitialized() const {
  ...
  for (int i = 0; i < forwardeddocuments_size(); i++) {
    if (!this->forwardeddocuments(i).IsInitialized()) return false;
  }
  return true;
}
